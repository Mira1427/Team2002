...
public:
	HighResolutionTimer()
	{
		...
		QueryPerformanceFrequency(reinterpret_cast<LARGE_INTEGER*>(&countsPerSec));		// 高精度カウンターの周波数の取得
		secondsPerCount_ = 1.0 / static_cast<double>(countsPerSec);						// 1秒あたりのカウンター数

		QueryPerformanceCounter(reinterpret_cast<LARGE_INTEGER*>(&thisTime_));			// 現在のカウンター値を取得
		baseTime_ = thisTime_;
		lastTime_ = thisTime_;
	}
