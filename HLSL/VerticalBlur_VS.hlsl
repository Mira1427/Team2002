...
PSBlurInput main(uint vertexID : SV_VertexID)
{
    ...
    vout.tex0_.xy = float2(0.0, 1.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+3テクセルずらすためのオフセットを計算する ---
    vout.tex1_.xy = float2(0.0, 3.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+5テクセルずらすためのオフセットを計算する ---
    vout.tex2_.xy = float2(0.0, 5.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+7テクセルずらすためのオフセットを計算する ---
    vout.tex3_.xy = float2(0.0, 7.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+9テクセルずらすためのオフセットを計算する ---
    vout.tex4_.xy = float2(0.0, 9.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+11テクセルずらすためのオフセットを計算する ---
    vout.tex5_.xy = float2(0.0, 11.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+13テクセルずらすためのオフセットを計算する ---
    vout.tex6_.xy = float2(0.0, 13.0 / texSize.y);
    
    // --- 基準テクセルからV座標を+15テクセルずらすためのオフセットを計算する ---
    vout.tex7_.xy = float2(0.0, 15.0 / texSize.y);
